...
class ProcessDenialCodes(DenialBase):
    ...
    def _extract_codes(self, denial_text: str) -> list[str]:
        ...
        return extract_loose_icd10_codes(denial_text) + extract_loose_cpt_codes(
            denial_text
        )
    ...
    async def get_denialtype(self, denial_text, procedure, diagnosis):
        ...
        for diag in extract_loose_icd10_codes(denial_text):
            tag = icd10.find(diag)
            if tag is not None:
                ...
                if diag in self.preventive_diagnosis:
                    return [self.preventive_denial]
        for code in extract_loose_cpt_codes(denial_text):
