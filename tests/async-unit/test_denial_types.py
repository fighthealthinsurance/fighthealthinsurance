...
class TestDenialTypes(TestCase):
    ...
    async def test_get_procedure_matches_asked_for_pattern(self):
        """get_procedure captures the procedure name from a fixture regex."""
    ...
    async def test_get_procedure_returns_none_when_no_match(self):
        """get_procedure returns None for text that matches no procedure regex."""
        checker = ProcessDenialRegex()
    ...
    async def test_get_diagnosis_matches_not_indicated_for(self):
        """get_diagnosis captures the diagnosis from a fixture regex."""
        checker = ProcessDenialRegex()
    ...
    async def test_get_diagnosis_returns_none_when_no_match(self):
        """get_diagnosis returns None for text that matches no diagnosis regex."""
        checker = ProcessDenialRegex()
    ...
    async def test_get_procedure_and_diagnosis_returns_tuple(self):
        """get_procedure_and_diagnosis returns both values for matching text."""
        checker = ProcessDenialRegex()
    ...
    async def test_get_appeal_templates_matches_covid_vaccine(self):
        """get_appeal_templates returns the COVID-19 template for a matching text."""
        checker = ProcessDenialRegex()
    ...
    async def test_get_appeal_templates_returns_empty_for_unrelated_text(self):
        """get_appeal_templates returns an empty list for text matching no template."""
        checker = ProcessDenialRegex()
        templates = await checker.get_appeal_templates(
    ...
    async def test_get_plan_type_matches_created_plan(self):
        """get_plan_type returns plans whose regex matches and whose negative_regex does not."""
        # Create two distinct plan types with non-empty regexes. Fixture
        # PlanType rows have blank regexes so they are non-selective.
        ...
        # HMO regex matches and there is no negative_regex → plan returned.
        ...
        # PPO regex matches but negative_regex also matches → excluded.
        ...
        # PPO regex matches and negative_regex does not → plan returned.
    ...
    async def test_get_regulator_matches_erisa_and_respects_negative(self):
        """get_regulator returns regulators whose regex matches and negative_regex does not."""
        ...
        # regex matches, negative_regex does not → regulator returned.
        ...
        # regex matches AND negative_regex matches → regulator excluded.
    ...
    async def test_process_denial_codes_icd_block_description_preventive(self):
        """Returns the preventive denial when an ICD-10 code's block description
        contains 'preventive'."""
    ...
    async def test_process_denial_codes_preventive_cpt_override(self):
        """Returns the preventive denial when a CPT code is in the
        preventive_codes CSV override."""
        processor = await sync_to_async(ProcessDenialCodes)()
    ...
    async def test_process_denial_codes_unrelated_text_returns_empty(self):
        """Returns [] when the denial text contains no recognizable codes."""
        processor = await sync_to_async(ProcessDenialCodes)()
