...
class SendFaxTest(APITestCase):
    ...
    def test_send_fax_rejects_fax_number_too_long_for_the_fax_row(self):
        """FaxesToSend.destination holds 20 characters, so 21 is a 400 up
        front rather than a failure after the number is saved."""
        url = reverse("appeals-send-fax")

        response = self.client.post(
            url,
            json.dumps({"appeal_id": self.appeal.id, "fax_number": "1" * 21}),
            content_type="application/json",
        )

        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.denial.refresh_from_db()
        self.assertEqual(self.denial.appeal_fax_number, "5551234567")
